...
#'
#' @param x some data
#' @return For Part One, `f02a(x)` returns numeric value
#'   For Part Two, `f02b(x)` returns ....
#' @export
#' @examples
...
f02a <- function(x) {
  # read in commands
  df <- read.table(text = x, col.names = c("direction", "units"))
  # sum up commands
  h <- sum(df$units[grepl("forward", df$direction)])
  d <- sum(df$units[grepl("down", df$direction)])
  u <- sum(df$units[grepl("up", df$direction)])
...
f02b <- function(x) {
  # read in commands
  df <- read.table(text = x, col.names = c("direction", "units"))
  # negate up units for easier summing
  df$units <- ifelse(df$direction == "up", -1 * df$units, df$units)
  # calculate cumulative aim
  df$aim <- cumsum(grepl("up|down", df$direction) * df$units)
  # calc final positions
  grepl_f <- expression(grepl("forward", df$direction))
  hz <- sum(df$units[eval(grepl_f)])
  dp <- sum(df$units[eval(grepl_f)] * df$aim[eval(grepl_f)])
  # horizontal * final depth
  hz * dp
}
